import"./style-yw6EFRbW.js";async function r(){try{const e=await fetch("http://localhost:3000/leaderboard");if(!e.ok)throw new Error(`HTTP error! status: ${e.status}`);const a=await e.json(),o=document.querySelector("#leaderboard-table tbody");if(!o)return;a.forEach(t=>{const d=document.createElement("tr");d.innerHTML=`
                <td>${t.rank}</td>
                <td>${t.name}</td>
                <td>${t.score}</td>
